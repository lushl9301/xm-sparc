...
struct xmHdr xmHdr __XMHDR = {.sSignature = XMEF_XM_MAGIC, .compilationXmAbiVersion =
		XM_SET_VERSION(XM_ABI_VERSION, XM_ABI_SUBVERSION, XM_ABI_REVISION), .compilationXmApiVersion =
		XM_SET_VERSION(XM_API_VERSION, XM_API_SUBVERSION, XM_API_REVISION), .noCustomFiles = 1,
		.customFileTab = {[0]={.sAddr = (xmAddress_t)physXmcTab, .size = 0, }, }, .eSignature = XMEF_XM_MAGIC, };
	...
	for (e = 0; e < xmcTab.noPartitions; e++) {
		if ((p = CreatePartition(&xmcPartitionTab[e]))) {
			...
			for (a = 0; a < xmcPartitionTab[e].noPhysicalMemoryAreas; a++) {
				...
				kprintf(" flags: 0x%x",
						xmcPhysMemAreaTab[a + xmcPartitionTab[e].physicalMemoryAreasOffset].flags);
				kprintf("\n");
			}
